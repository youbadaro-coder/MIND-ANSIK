        ...
        for video in results.get('videos', []):
            ...
            for f in video_files:
                if f['file_type'] == 'video/mp4':
                    ...
                    # Cap at 1920px to avoid massive 4K downloads that exhaust CPU compositing
                    if width <= 1920:
                        if not best_file or width > best_file.get('width', 0):
                            best_file = f
            
            # Fallback if all files are somehow > 1920
...
def fetch_materials():
    ...
    input_path = os.path.join(input_dir, 'topic_data.json')
